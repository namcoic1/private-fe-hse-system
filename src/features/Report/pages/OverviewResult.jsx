...
function OverviewResult() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const actionResult = await serviceApi.getServiceDetail(serId);
        if (actionResult.success) {
          setService(actionResult.data);
        }
      } catch (error) {
        if (error.response.status === 401) {
          try{const actionRefresh = await userApi.refreshToken();}catch(err){if(err.response.status===401){window.location.href = "/authentication/signin";}}
          const actionResult = await serviceApi.getServiceDetail(serId);
          if (actionResult.success) {
            setService(actionResult.data);
          }
